...
interface OrderDao : IDao<OrderEntity> {
    ...
    @Transaction
    @Query("SELECT id, subject_id, remark, order_status, payment_status, create_time FROM `order` WHERE client_id = :clientId ORDER BY create_time DESC")
    fun observeOrderProfileListByClientId(clientId: Long): Flow<List<OrderProfileDto>>

    @Transaction
    @Query("SELECT id, subject_id, remark, order_status, payment_status, create_time FROM `order` WHERE character_id = :characterId ORDER BY create_time DESC")
