...
interface AccountDao : IDao<AccountEntity> {
    ...
    @Transaction
    @Query("SELECT id, username, ( SELECT Count(*) FROM CHARACTER WHERE account_id = account.id ) AS character_count, ( SELECT Count(*) FROM `order` WHERE account_id = account.id ) AS order_count, create_time FROM account WHERE client_id = :clientId ORDER BY create_time DESC")
